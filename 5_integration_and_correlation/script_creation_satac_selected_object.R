...
# saveRDS(object, "output/sATAC_preprocessing/brca_unmerged.rds")
...
# %%
# signac_object[[1]]$peaks@data

# %%
# signac_object[[1]]$peaks@counts

# %% [markdown]
# not-normalised with LSI.
...
# write.csv(signac_filtered@assays$peaks@counts,
#           "./output/sATAC_preprocessing/counts_signac_filtered_brca_q0_peak_bc_matrix.csv")

# %%
# new
# signac_filtered@assays$peaks
...
#     - for counts
# ```sh
# nohup dca output/sATAC_preprocessing/counts_signac_filtered_brca_q0_peak_bc_matrix.csv output/sATAC_preprocessing/counted_dca_peaks_q0_brca --threads 3 --nosizefactors --nonorminput --nologinput --saveweights > nohup_for_dca_counts.out &
# ```
#   - for prenormalised data:
# ```sh
# nohup dca output/sATAC_preprocessing/data_signac_filtered_brca_q0_peak_bc_matrix.csv output/sATAC_preprocessing/dated_dca_peaks_q0_brca --threads 3 --nosizefactors --nonorminput --nologinput --saveweights --nocheckcounts > nohup_for_dca_data.out &
# ```
# 
...
# peaks
denoised_counts <- read.table("./output/sATAC_preprocessing/dated_dca_peaks_q0_brca/mean.tsv", row.names = 1) %>% 
  as.matrix()
...
dim(signac_filtered[["peaks"]]@data)
...
saveRDS(signac_filtered, "output/sATAC_preprocessing/filtered_brca_denoised_peaks.rds")
...
# %%
saveRDS(signac_filtered, "output/sATAC_preprocessing/filtered_compared_brca_denoised_peaks.rds")
...
write.csv(signac_filtered@assays$RNA@counts, "output/sATAC_preprocessing/gene_activity_counts_brca.csv")
