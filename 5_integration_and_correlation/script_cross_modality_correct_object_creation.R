...
# %%
signac_selected[["dca"]] <- CreateChromatinAssay(data = as.matrix(denoised_counts))

# %%
# signac_selected[["dca"]] <- signac_selected[["peaks"]]

# %%
# signac_selected@assays$dca@data <- as.matrix(denoised_counts)

# %%
# head(as.matrix(denoised_counts))
# head(signac_selected@assays$dca@data)

# %%
# create dca by copying the peaks assay
# signac_selected[["dca"]] <- signac_selected[["peaks"]]
...
# %%
# signac_selected <- readRDS("output/sATAC_preprocessing/data_coorectly_in_object.rds")

# %%
# # create dca_1 as a temporal importing assay
# signac_selected[["dca_1"]] <- CreateAssayObject(data = as.matrix(denoised_counts))
...
# # Copy relevant metadata and computational slots from the "peaks" assay
# signac_selected[["dca_1"]]@meta.features <- signac_selected[["peaks"]]@meta.features  # Gene metadata
# signac_selected[["dca_1"]]@var.features <- signac_selected[["peaks"]]@var.features  # Variable features
# signac_selected[["dca_1"]]@scale.data <- signac_selected[["peaks"]]@scale.data  # Scaled data (if used)

# # Now, copy Signac-specific slots
# signac_selected[["dca_1"]]@fragments <- signac_selected[["peaks"]]@fragments  # Peak accessibility fragments
# signac_selected[["dca_1"]]@annotation <- signac_selected[["peaks"]]@annotation  # Genomic annotations
# signac_selected[["dca_1"]]@positions <- signac_selected[["peaks"]]@positions  # Genomic positions index

# # Ensure all links to Signac objects remain intact
# signac_selected[["dca_1"]]@links <- signac_selected[["peaks"]]@links  # Peak-to-gene links (if available)
# signac_selected[["dca_1"]]@motifs <- signac_selected[["peaks"]]@motifs  # Motif analysis results (if applicable)
...
signac_selected <- RunTFIDF(signac_selected, assay = "dca", verbose = FALSE) %>%
  FindTopFeatures(min.cutoff = 'q0') %>%
  RunSVD()
...
# %%
# signac_selected <- signac_selected %>%
#   RunUMAP(reduction = "lsi", dims = 2:35, verbose = FALSE) %>%
#   FindNeighbors(reduction = "lsi", dims = 2:35, verbose = FALSE) %>%
#   FindClusters(resolution = 0.5, verbose = FALSE, cluster.name = 'seurat_clusters_dca_1') 
...
# %%
# head(signac_selected@assays$dca@data)
# head(signac_selected@assays$peaks@data)

# %%
# signac_selected$dca_1_snn_res.0.5 <- signac_selected$seurat_clusters_dca_1
...
# %%
# prop.table(table(signac_selected$peaks_snn_res.0.5, signac_selected$dca_1_snn_res.0.5), 1)*100

# %%
# Check the proportion of spots assigned to each cluster when using denoised vs original data
prop <- prop.table(table(signac_selected$peaks_snn_res.0.5, signac_selected$seurat_clusters_dca_1), 1)*100
...
# gene activity
# signac_selected[["dca"]] <- NULL
mtx <- read.table(paste0(PATH_sATAC_PREPROCESSING_OUTPUT, "dca_gene_activity_selected_", SELECTED_sATAC_SAMPLE, "_brca2/mean.tsv"))
